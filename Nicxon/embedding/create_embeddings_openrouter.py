#!/usr/bin/env python3
"""
Library Chatbot Embedding Generation Script (OpenRouter)
...
class BookEmbeddingGenerator:
    def __init__(self, api_key: str = None, model_name: str = "openai/text-embedding-3-small"):
        ...
        self.embedding_dim = None  # Will be detected from test call
        ...
        self.min_delay = 0.01  # Minimum delay between API calls (10ms)
    ...
    def test_api_connection(self):
        ...
        try:
            ...
            if response and response.data and len(response.data) > 0:
                ...
                if response.usage:
                    print(f"📊 Test tokens used: {response.usage.total_tokens}")
            else:
    ...
    def generate_statistics_report(self, output_file: str = "embedding/embedding_statistics_openrouter.txt"):
        """Generate detailed statistics report"""
        
        duration = self.stats['processing_end'] - self.stats['processing_start']
        
        report = []
        report.append("LIBRARY CHATBOT EMBEDDING STATISTICS (OpenRouter)")
        report.append("=" * 50)
...
def main():
    ...
    print("🚀 Library Chatbot Embedding Generator (OpenRouter)")
    print("=" * 50)
    ...
    if model_choice == "4":
        model_name = input("Enter model ID (e.g. 'openai/text-embedding-3-small'): ").strip()
    else:
    ...
    try:
        generator = BookEmbeddingGenerator(api_key, model_name=model_name)
    except Exception as e:
    ...
    output_file = "embedding/book_embeddings_openrouter.json"
    ...
    try:
        ...
        print(f"📊 Statistics saved to: embedding/embedding_statistics_openrouter.txt")
